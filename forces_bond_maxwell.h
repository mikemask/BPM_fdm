...
//    ttr = bond -> getTorque_tr();
    ...
    S_old[0] = r_old[1]*(r_old[0]*r_0[1]-r_0[0]*r_old[1]) - r_old[2]*(r_0[0]*r_old[2]-r_old[0]*r_0[2]);
    S_old[1] = r_old[2]*(r_old[1]*r_0[2]-r_0[1]*r_old[2]) - r_old[0]*(r_0[1]*r_old[0]-r_old[1]*r_0[0]);
    S_old[2] = r_old[0]*(r_old[2]*r_0[0]-r_0[2]*r_old[0]) - r_old[1]*(r_0[2]*r_old[1]-r_old[2]*r_0[1]);

    double vc_n[3], vc_s, vc_s_old;

    double mod_S = modulus(S);
    double mod_Sold = modulus(S_old);
    ...
    {
        s_old[0] = S_old[0]/mod_Sold;
        s_old[1] = S_old[1]/mod_Sold;
        s_old[2] = S_old[2]/mod_Sold;
    }

    vc_s = v_c[0]*s[0] + v_c[1]*s[1] + v_c[2]*s[2];
    vc_s_old = vc_old[0]*s_old[0] + vc_old[1]*s_old[1] + vc_old[2]*s_old[2];
...
//        fs[0] = 1./(1./(ks*area) + dt/mus)*vc_s*dt*s[0];
//        fs[1] = 1./(1./(ks*area) + dt/mus)*vc_s*dt*s[1];
//        fs[2] = 1./(1./(ks*area) + dt/mus)*vc_s*dt*s[2];
    }
...
//        fs[2] = (fs_old/(1. + dt/lambdas) + 1./(1./(ks*area) + dt/mus)*vc_s*dt)*s[2];
        ...
        fs[0] = (fs_old/(1. + dt/lambdas) + 1./(1./(ks*area) + dt/mus)*mod_r0*(sin(beta) - sin(beta_old)))*s[0];
        fs[1] = (fs_old/(1. + dt/lambdas) + 1./(1./(ks*area) + dt/mus)*mod_r0*(sin(beta) - sin(beta_old)))*s[1];
        fs[2] = (fs_old/(1. + dt/lambdas) + 1./(1./(ks*area) + dt/mus)*mod_r0*(sin(beta) - sin(beta_old)))*s[2];
    }
